...
bin — утилиты для компиляции
	win32 или win64
		...
		GoRC.exe — компилятор ресурсных скриптов
...
Почему же не сделать всё за один шаг? Для простейших программ это действительно было бы проще, но для сложных проектов многоступенчатый процесс имеет преимущества:

* сложная программа разбивается на несколько отдельных частей (модулей), которые отлаживаются отдельно и зачастую разными людьми, поэтому в завершении остаётся лишь собрать готовые модули в единый проект;
...
<pre><samp><span class="promt">C:\Users\Пользователь&gt;</span><kbd>cd %programfiles%\freebasic</kbd>
...
<pre><samp><span class="promt">C:\Program Files\FreeBASIC&gt;</span><kbd>fbc "C:\Programming\FreeBASIC Projects\HelloWorld.bas"</kbd>
</pre></samp>

Совет: можно перетащить значок нужного файл из проводника прямо в консоль.
...
```Batchfile
"%ProgramFiles%\FreeBASIC\fbc.exe" %1
```

Теперь можно создавать программы перетаскивая мышкой исходные файлы на этот пакетный файл.
...
<pre><samp><span class="promt">C:\Programming\FreeBASIC Projects&gt;</span><kbd>HelloWorld.exe</kbd>
Привет, мир!
...
<pre><samp><span class="promt">C:\Programming\FreeBASIC Projects&gt;</span><kbd>PrintSomeWords.exe</kbd>
