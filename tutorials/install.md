...
Есть несколько вариантов сборок с разными средствами разработки и битности.
...
Продвинутый блокнот с подсветкой синтаксиса ключевых слов, встроенных функций и автоматической расстановкой отступов.
...
1. Компиляция (трансляция) — перевод текста программы в код промежуточного представления для Back-end. По умолчанию Back-end выступает GCC, и FreeBASIC генерирует промежуточное представление кода на языке Си.
2. Back-end транслирует сгенерированный код в ассемблерный код. (На 32‐битной системе фрибейсик по умолчанию создаёт ассемблерный код непосредственно, без компиляции в промежуточное представление.)
3. Программа‐ассемблер из ассемблерного кода создаёт объектные файлы.
4. Компоновщик связывает все сгенерированные объектные файлы и код функций из статических библиотек в один исполняемый файл.
5. Фрибейсик удаляет временные файлы.
...
Почему же не сделать всё за один шаг? Такой ступенчатый процесс имеет преимущества. Вместо того, чтобы делать компилятор полностью для каждой новой платформы, можно оставить front-end неизменным (он отвечает за синтаксис языка) и написать только новый back-end. Более того, вместо того, чтобы использовать собственную разработку, FreeBASIC использует широко известные решения. В качестве такого известного решения разработчики FreeBASIC решили использовать Gnu Compiler Collection (GCC) и Low Level Virtual Machine (LLVM).
...
│       │
│       ├───gdb.exe — отладчик (поставляется отдельно в 64‐битной версии)
│       │
│       ├───gcc.exe — back-end GCC (поставляется отдельно в 32‐битной версии)
│       │
