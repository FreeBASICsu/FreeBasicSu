...
* при записи в файл теряется начальная кавычка «"», при чтении — исчезает начальные и конечные кавычки;
* отсутствует настройка доступа: ты не можешь сказать «кто угодно может редактировать эту секцию, а вот этот раздел могут изменять только администраторы»;
