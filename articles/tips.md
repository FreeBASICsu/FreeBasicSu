...
«Ёмкость» переменной ограничена типом данных, мы не можем хранить в переменной бесконечно большое число. Например, максимально возможное число, которое «влезет» в переменную типа `Long` ограничена константой `2147483647`. Если мы к такой переменной прибавим единицу, то произойдёт переполнение.

Переполнение знаковой переменной — это неопределённое поведение. Задача программиста должен писать программу так, чтобы в коде не встречалось неопределённое поведение. Со своей стороны компилятор полагает, что в коде не встретится неопределённого поведения. Поэтому для знаковой переменной компилятор предполагает, что она не будет переполнена.

Целый класс оптимизаций компилятора опирается на отсутствие переполнения знаковых переменных.

С другой стороны, переполнение беззнаковых переменных не является неопределённым. Когда мы используем беззнаковые переменные в счётчиках цикла, то компилятор не может применить такие оптимизации.
...
```FreeBASIC
' Циклы со знаковыми переменными работают быстрее:
